...
$idU=$_SESSION['idU'];
$idA=$_SESSION['idA']; // 1admin 2 cliente
...
var cantidad=document.header2.cantidad.value;
var sub=0;
var precio=0;
...
x.id="p"+elementos[0];
y.innerHTML="<center>"+document.header2.cantidad.value+"</center>";
...
z3.innerHTML="<input name=\"precio[]\"  type=\"hidden\" value=\""+precio+"\" /><input name=\"mayoreo[]\"  type=\"hidden\" value=\"0\" /> <input name=\"idproducto[]\"  type=\"hidden\" value=\""+elementos[0]+"\" /><input name=\"cantidad_d[]\"  type=\"hidden\" value=\""+document.header2.cantidad.value+"\" /><input name=\"monto[]\"  type=\"hidden\" value=\""+sub+"\" /><input name=\"descripcion[]\"  type=\"hidden\" value=\""+descri+"\" /><img src=\"images/close.gif\" alt=\"Eliminar Producto\" name=\"Image50\"  border=\"0\"  id=\"Image50\" onclick=\"deleteRow(this,"+sub+")\"/>";
...
elementos= cadena.split("|");
		 ...
		 precio.innerHTML=elementos[5];//muestra costo en label con id=precio2
...
{	
	...
	$query = "SELECT * FROM productos where codigo_barras='$barras' and costo!='0'";
	$result = mysql_query($query) or print("error $query".mysql_error());
	if($res_marca = mysql_fetch_assoc($result)){
		$dato=$res_marca['id_producto']."|".$res_marca['nombre']."|".$res_marca['descripcion']."|".$res_marca['precio']."|".$res_marca['cantidad']."|".$res_marca['costo'];		
				
		echo"showDes('$dato');parent.insRow('$dato',$c);";
	}
	else
	{
