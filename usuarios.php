...
<title>Tienda</title>
 ...
 function borrar(id, nombre){
        if(confirm('Deseas eliminar el usuario: '+nombre+'?')){
            ...
            $("#form1").attr('action','elimina_usuario.php');
            $("#form1").submit();
        }
    }
</script>
   ...
   <!--tabla-->
	     ...
	     <table class="centered bordered highlight white-text">
		    <thead>
			  <tr>
				...
				<th>Telefono</th>
				<th>Sucursal</th>
				<th colspan="2"><a href="nuevo_usuario.php" ><i class="fa fa-plus"></i> Nuevo Usuario</a></th>
			  </tr>
			</thead>
			<tbody>
	<? $query = "SELECT usuarios.id_usuarios,usuarios.nombre,usuarios.correo,usuarios.telefono,(sucursales.nombre) as sucursal 
	             FROM usuarios
	...
	?>		
          <tr>
            ...
            <td><? echo $res_prod['correo']?></td>
			<td><? echo $res_prod['telefono']?></td>
			 <td><? echo $res_prod['sucursal']?></td>
			 <td><a href="editar_usuario.php?id=<? echo $res_prod['id_usuarios'];?>">
			 <i class="fa fa-pencil fa-lg"></i></a></td>
	<td><a href="javascript:borrar(<? echo $res_prod['id_usuarios']?>, '<? echo $res_prod['nombre']?>', '<? echo $res_prod['foto']?>');">
