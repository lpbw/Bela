...
function abrir2(id)
{
$.colorbox({iframe:true,href:"editar_inventario.php?id="+id,width:"600", height:"500",transition:"fade", scrolling:false, opacity:0.7});	
}	
</script>
</head>
       ...
       <!--inicio select sucursal-->
	    <div class="input-field col s4 m4 l4 xl4">
          <select class="white-text" name="sucursal" id="sucursal"/>
		    ...
		    <option value="<? echo $res_suc['id_sucursal']?>"<? echo $idsuc==$res_suc['id_sucursal']?"selected":""; ?>><? echo $res_suc['nombre']?></option>
		     <?
               }
             ?>
          ...
          <tr>
			...
			<td><? echo $res_prod['codigo_barras']?></td>
			 <td><? if($res_prod['foto']!=""){?><img src="images/<? echo $res_prod['foto']; ?>" width="90" height="90" /><? }?></td>
			  <td><a href="javascript:abrir2(<? echo $res_prod['id_inventario']?>)"><i class="fa fa-pencil fa-lg"></i></a></td>
          </tr>
